...
新版本采集还会生成私有 `output/<周次>/raw/all_reports.json`，用于 Web 中“下载原周报”。它只包含指定周报模板和旧“周报”模板，不参与提交统计或 AI 评价。升级前已经采集的周次没有该快照；需要下载这些历史周次时，必须在服务器重新运行对应周次采集，系统不会用不完整数据静默生成文件。

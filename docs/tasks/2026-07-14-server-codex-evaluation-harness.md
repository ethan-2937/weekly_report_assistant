...
- 剩余风险：服务器 Codex 登录状态、CLI 版本、模型额度、网络/代理和 Office/PDF 解析工具需要部署时验证；Harness 会在新旧 CLI 间选择审批参数，并通过私有 `WEEKLY_CODEX_BASE_URL` 显式配置公司 `crs`/`responses` provider；cron 还必须提供私有 `CRS_OAI_KEY`；输入变化后的评价是完整重建，不做脆弱的局部文本补丁。

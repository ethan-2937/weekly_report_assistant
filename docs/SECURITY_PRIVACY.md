...
- 原周报 XLSX 属于高敏感下载：仅从服务器私有 `raw/all_reports.json` 读取，先执行当前账号服务层范围过滤，再保留授权人员的 userid、原始字段和重复提交。文件不包含 report_id、内部路径、原始 JSON、地址、评论、点赞或阅读统计；缺少完整双模板快照、身份字段或内容超过 XLSX 单元格限制时拒绝生成。前端请求不携带正文或人员清单，普通日志只记录通用失败，不记录文件内容。

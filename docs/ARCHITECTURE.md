...
- 正式发送和管理员复核共用 `EvaluationFeedbackMessageFormatter`，避免页面正文与钉钉正文漂移。新的完成状态保存不含正文的反馈摘要；旧状态以私有反馈文件更新时间不晚于发送完成时间作为兼容判断。`GET /api/admin/evaluation-feedback-previews/{week}` 仅在完成状态、人数和一致性校验通过后返回单周正文，不返回 userid、文件路径或原始周报。

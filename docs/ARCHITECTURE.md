...
- Python 负责外部采集、周次计算、提交归属窗口、人员匹配、免交名单过滤、负责人附件下载和生成确定性输入包；`submission_roster.py` 应用服务器私有免交配置，`submission_reminder.py` 只生成截至当前时刻的轻量候选快照，不写正式周报输出，`attachment_download.py` 通过钉钉 Drive 短期下载信息受控落盘，`leader_compliance.py` 生成履职证据，`project_details.py` 提取不拆分的七列项目明细，`weekly_outputs.py` 统一生成 CSV/Markdown。

...
- 同一次自动评价还生成私有 `employee_feedback.json` v2，包含表扬、改进和基于本人当周证据的两句式感谢；Harness 要求反馈 userid 集合与已提交集合完全一致，并拒绝姓名、其他人员信息、secret 形态和超长正文。Spring 兼容 v1 文件并使用固定暖心感谢兜底；周一通知不解析 Markdown，也不再次调用 Codex。
- Codex Skill 负责从授权输入生成管理评价，不负责身份认证或数据权限。
- Spring Boot 负责身份、数据范围、文件读取、任务触发、周日定时编排和钉钉通知。周日提醒及管理员结果固定使用工作通知；周一个性化 Feedback 由同一适配器在默认 `WORK_NOTICE` 与显式 `ROBOT_OTO` 中择一发送，机器人单聊使用 `BatchSendOTO` 且不做跨通道自动回退。
- Spring 周一评价通知读取上一完整周的成功评价状态、正式报告摘要、私有反馈清单和提交状态；候选提供器复用 `TemplateComplianceService` 从同一原始周报快照补充提交概览使用的百分比，缺失或越界即失败关闭。每名员工单独发送，结果不确定时通过无明细状态阻止自动重发。
- 正式发送和管理员复核共用 `EvaluationFeedbackMessageFormatter`，因此两处都展示同源模板符合度并避免正文漂移。新的完成状态保存不含正文的反馈摘要；旧状态以私有反馈文件更新时间不晚于发送完成时间作为兼容判断。`GET /api/admin/evaluation-feedback-previews/{week}` 仅在完成状态、人数和一致性校验通过后返回单周正文，不返回 userid、文件路径或原始周报。
...
- Vue 运行状态页仅在 ADMIN 视图中按需调用单周 Feedback 复核接口；切换周次或请求失败会清除旧响应，通知正文使用纯文本呈现而不执行 Markdown HTML。
...
管理员通知复核使用 `GET /api/admin/evaluation-feedback-previews/{week}`，无请求体。成功响应包含周次、发送阶段、应发/已发人数、完成时间、一致性状态和逐人通知正文；400 表示周次参数无效，401 表示未认证，403 表示非 ADMIN，409 表示未完整发送或无法安全复核，500 仅返回通用错误。接口不接受或返回 userid。
